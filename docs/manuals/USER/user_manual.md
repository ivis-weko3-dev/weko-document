...
システムにログインおよびログアウトするための操作手順を説明しています。
...
この章では、システムにログインおよびログアウトする方法について説明します。
...
| 6  | ［パスワードをお忘れの方はこちら］リンク                        | クリックすると、パスワードの再設定ができます。                                                      |
...
ログインに失敗した場合のエラーメッセージ。

  - 既に同一のメールアドレスが登録されている場合など：「ユーザ情報の更新に失敗したため、ログインできませんでした。」

  - 共有アカウントのIP範囲外の場合：「法人用の共有アカウントが、契約外のIPアドレスから利用されているためにログインできません。契約されている機関内のIPレンジからご利用ください。」
...
コンテンツファイルへのアクセス権が無い場合の挙動については、「4.1 アイテム詳細画面」の「プレビュー表示でコンテンツファイルの公開日が未来日の場合」を参照してください。

コンテンツファイルが課金ファイルだった場合の挙動については、「4.1　アイテム詳細画面」 の「課金ファイルが設定されている場合」の「「Charge」ボタンを押した場合」を参照してください。
...
・公開日が未来日の状態のコンテンツファイルの［Download］ボタンや［Preview］ボタンが押された場合、ログインをするかどうか確認するメッセージを表示します。ログインしたロールに権限が無い場合にエラーメッセージを表示します。（アイテムリストの拡張子リンクやアイテム詳細画面のファイルリンク、ファイルのリンクから直接アクセスした場合も同様）
...
2.  ログインしているユーザの持つロールが設定されている課金可能なロールに該当する場合、そのうちの最安値の価格情報がハイライトされます。<br>
    ...
    サイトライセンス認可でダウンロード可能な場合は、価格情報の箇所に「サイトライセンス会員: ￥0」を表示します。<br>

![](media/media/image300.png)

3.   情報学広場で、ファイルに公開日が設定されていて、公開日を過ぎた場合は全ロールで「オープンアクセス」を表示します。
![](media/media/image308.png)

4.  「Charge」ボタンを押した場合（アイテムリストの拡張子リンクやアイテム詳細画面のファイルリンク、ファイルのリンクから直接アクセスした場合も同様）

ログインをしていない場合、ログインをするかどうか確認し、［OK］ボタンを押した場合にログイン画面を表示します。ログインをしたロールが課金可能かどうかで表示されるメッセージが異なります。

![](media/media/image307.png)

図 4-23　課金ファイルの価格情報表示

課金可能でないロールでログインをした場合、「ダウンロード権限がありません」としてエラーメッセージが表示されます。

![](media/media/image306.png)

図 4-24　課金ファイルの価格情報表示

課金可能なロールでログインをした場合、課金を行うか確認するメッセージが表示されます。<br>

![](media/media/image302.png)

「Charge」ボタンを押した場合：課金が行われます。場合によっては、3Dセキュアの認証画面が表示されます。(広場)<br>
　　　　　　　　　　　　　課金が成功した場合、ファイルがダウンロードされます。<br>
「Cancel」ボタンを押した場合：アイテム詳細画面に戻ります。<br>
「Charge」ボタンは課金設定に該当のロールが設定されていないユーザの場合、表示しません。

図 4-25　課金ファイルの価格情報表示
...
図 4-32 Information画面

![](media/media/image114.png)

図 4-33 Information画面の統計(Stats)タブ選択状態
