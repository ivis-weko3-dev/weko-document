...
[9.8 非公開コンテンツをワンタイムアドレスを利用して共有したい nnn](#非公開コンテンツをワンタイムアドレスを利用して共有したい)
...
<!--
＊以下の操作は、制限公開機能の先行利用に参加している機関のみ行うことができます。

・コンテンツファイルのアクセスが「制限公開」と設定されているコンテンツはアクセスコントロールが設定されており、利用規約の同意等の手続きを行った後に# コンテンツをダウンロードすることができます。制限公開の設定については「5.1.1　アイテムを登録する」を参照してください。
...
> 図 512\[ユーザ情報入力項目\]画面 Owner選択 

Usernameの左側のラジオボタンをいずれかのユーザから選択し（１名の場合はそのユーザ）、Ownerに設定します。 

代理投稿者として指定するユーザには必ずログインユーザを指定します。 

入力したログインユーザの行を削除しようとするとき［ログインユーザーは削除できません。］というエラーメッセージが表示されます。 
...
**<span style="color: red; ">本機能は、公開コンテンツ、ならびにサイトライセンス機能を利用してサイトライセンスユーザーに対してアクセス可能になっているコンテンツに対して、新たにアクセス権限を付与し、シークレットURLを通じたアクセスに制限する機能ではありません<span>。**
 ...
 下記の条件を満たすことで、Information画面のファイルエリアに「Secret URL」ボタンが追加されるのは、以下の条件を満たした場合となります。
