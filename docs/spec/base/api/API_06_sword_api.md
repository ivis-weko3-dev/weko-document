...
4. 登録処理を行う
    ...
    **ワークフロー経由で登録する場合**
    ...
    - アイテムのPIDと、登録が完了した場合はレコードのURL、アクティビティ詳細画面のURL、ファイル情報がある場合ファイルリンクを返却する。
...
5. レスポンスを返却する
   - アイテムの登録が完了した場合は、登録されたアイテムのURLおよび、ファイル情報がある場合はそのファイルのURLをステータスドキュメントに含めて返却する。
   - アイテムの登録が完了していない場合は、アクティビティのURLおよび、ファイル情報がある場合はそのファイルのURLをステータスドキュメントに含めて返却する。
...
22. SWORD APIを使用することができるロール<span id="conf22">
    ...
    ```
23. ファイルセットファイルのファイルパス<span id="conf23">
