...
| wk:feedbackMail                            | .FEEDBACK_MAIL       | 配列（要素は文字列）| -            |      |      | フィードバックメール                     |
| wk:requestMail                             | .REQUEST_MAIL        | 配列（要素は文字列）| -            |      |      | リクエストメール                         |
...
| wk:itemLinks.identifier                    | -                    | URL or 文字列       | -            |      |      | アイテムリンク先識別子                   |
...
アイテムに紐づけるリクエストメールアドレスを指定する。ルートデータセット直下に記述する。
...
アイテムは`{"@id": "_:item1"}`と`{"@id": "_:item2"}`の2つに独立した状態で分割されるため、メタデータはそれぞれのアイテムに記述する。  
また、相互のアイテムは`wk:itemLinks`プロパティを使用してアイテムリンクを設定する。
分割後のアイテムをリンク先とするアイテムリンクに設定できるリレーションタイプは`isSupplementTo`, `isSupplementedBy`のみである。  
分割後のアイテム間で相互にアイテムリンクを作成した際、アイテム詳細画面のアイテムリンクはリンク先の承認状態に関わらず表示される。
リンク先に遷移するとリンク先のアイテムが未承認の場合はページ未検出と表示され、承認済の場合はアイテム詳細画面が表示される。
