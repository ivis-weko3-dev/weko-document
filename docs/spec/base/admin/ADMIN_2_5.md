...
- アイテムのメタデータをTSVインポートと同様の[チェック仕様](./ADMIN_2_4.md#4-チェック仕様)でバリデーションチェックを行う。
...
4. 識別子変更モード（Identifier Change Mode）チェックボックス
...
## メタデータ記述
RO-Crate 1.1に対応していて、アイテムのメタデータは`ro-crate-metadata.json`ファイルにJSON-LD形式で記述される。  
...
アイテムIDとURIがファイル内で指定されていない場合は、リクエストURLのパスパラメータで指定されたアイテムID（recid）を用いて、アイテムIDとURIの値を自動的に補完する。
...
`identifier`はアイテムリンク先のアイテムURLを指定し、`value`はアイテムリンクのリレーションタイプを指定する。  
